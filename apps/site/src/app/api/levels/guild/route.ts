...
export const GET = async (request: NextRequest) => {
  ...
  const authorisationCode = (request.headers.get('Authorization')?.startsWith('Bot ') || request.headers.get('Authorization')?.startsWith('Bearer '))
  ...
  if (!authorisationCode) return NextResponse.json({
    error: 'Unauthorised',
  ...
  const isAuthorised = isPhaseBot ? true : (!!authorisedUserSchema && !!(authorisedUserSchema.guilds.find((guild) => guild.id == guildId)))

  if (!isAuthorised) return NextResponse.json({
    error: 'Unauthorised',
    documentation: `${process.env.BASE_URL}/docs/api/authorisation`,
  }, { status: 401 })
  ...
  if ((!startIndex && startIndex != 0) || (!endIndex || endIndex <= 0 || endIndex > 15) || !guildId) return NextResponse.json({
    error: 'Bad Request',
  ...
  try {
    ...
    if (!guildLevelData) return NextResponse.json({
      error: 'Not Found',
      message: `No matching data found.`,
    }, { status: 404 })

    const levelsArray = guildLevelData.levels
    const levelsArraySorted = levelsArray.sort((a, b) => a == b ? b.xp - a.xp : b.level - a.level)

    const levelsData: any[] = []

    for (const levelData of levelsArraySorted.slice(startIndex, endIndex + 1)) levelsData.push(await discordAPI.users.get(levelData.id))
...
type UserLevelObject = discord_api_types_v10.APIUser & { level: number; xp: number; target: number; rank: number; }
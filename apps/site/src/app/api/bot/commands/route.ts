...
export const GET = async (request: NextRequest) => {
  ...
  const globalCommands = await discordAPI.applicationCommands.getGlobalCommands(process.env.DISCORD_ID!)
  ...
  for (const globalCommand of globalCommands) {
    ...
    if (globalCommand.options) for (const globalCommandOption of globalCommand.options) {
      ...
      commandArray.push({
        ...
        options: globalCommand.options.map((option) => {
          if (option.type != discord_api_types_v10.ApplicationCommandOptionType.Subcommand) return option as discord_api_types_v10.APIApplicationCommandOption
        }),
        nsfw: globalCommand.nsfw,
      })

      if (globalCommandOption.type == discord_api_types_v10.ApplicationCommandOptionType.Subcommand) commandArray.push({

...
export const GET = async (request: NextRequest) => {
  ...
  if (cookies().get('auth_session')?.value) await redis.del(`auth:${cookies().get('auth_session')?.value}`)

  cookies().set('auth_session', discordUserData.session, { sameSite: true, secure: true })
  cookies().set('auth_id', discordUserData.identity.id, { sameSite: true, secure: true })

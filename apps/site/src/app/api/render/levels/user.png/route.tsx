...
export async function GET (request: NextRequest) {
  ...
  const authorisationCode = authorisationHeader ?? `Bearer ${authorisationCookie}`


  const userLevelDataRequest = await fetch(`${process.env.BASE_URL}/api/levels/user?user=${userId}&guild=${guildId}&time=${Date.now()}`, { headers: { 'Authorization': authorisationCode } })
  if (!userLevelDataRequest.ok) return NextResponse.json(await userLevelDataRequest.json(), { status: userLevelDataRequest.status })
  ...
  const imageResponseTSX = (
    ...
    >
      <img
        src={userLevelData.avatar ? discordREST.cdn.avatar(userLevelData.id, userLevelData.avatar, { extension: 'jpg', size: 256 }) : '/discord.png'}
        alt='User Avatar'
  ...
  const poppinsFontResponse = await fetch('https://github.com/google/fonts/raw/main/ofl/poppins/Poppins-Bold.ttf')

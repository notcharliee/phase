...
export async function GET(request: NextRequest) {
  ...
  try {
    const dataResponse = await fetch(
      `${env.NEXT_PUBLIC_BASE_URL}/api/levels/user?user=${userId}&guild=${guildId}&t=${Date.now()}`,
    )
    ...
    const geistSemiboldResponse = await fetch(
      `${env.NEXT_PUBLIC_BASE_URL}/fonts/Geist-600.otf`,
    )

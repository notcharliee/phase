...
export async function GET (request: NextRequest) {
  ...
  try {
    const dataResponse = await fetch(absoluteURL(`/api/levels/guild?guild=${guildId}&rankStart=${rankStart}&rankEnd=${rankEnd}&t=${Date.now()}`))
    ...
    return new ImageResponse((
      <div style={{
        ...
        {await Promise.all(data.map(async (user, index) => (
          <div key={index} style={{
            ...
            <img src={user.avatar} width={60} height={60} style={{
      ...
      fonts: [
        {
          data: await (await fetch(absoluteURL("/fonts/Geist-600.otf"))).arrayBuffer(),
          name: "Geist",
          style: "normal",
          weight: 600,
        },

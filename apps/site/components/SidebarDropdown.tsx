...
export default ({ icon, title, items, disabled }: { icon: FaSolid.IconDefinition, title: string, items?: React.ReactElement[], disabled?: boolean }) => {
  ...
  return (
    <div>
      <div
        className={'w-full bg-dark-700 font-medium p-3 pl-4 pr-4 rounded select-none shadow border border-dark-600 flex items-center' + `${disabled
      ...
      >
        ...
        {title}
        <FontAwesomeIcon icon={FaSolid.faAngleDown} className='ml-auto w-4 h-4 float-right duration-150' style={{ transform: linksVisibility ? 'rotate(180deg)' : 'rotate(0deg)' }}></FontAwesomeIcon>
      </div>

...
export const GET = async (request: NextRequest) => {
  ...
  try {
    ...
    const guildLevelData = await Schemas.Levels.findOne({ guild: guildId, levels: { $elemMatch: { id: userId } } })
    ...
    const userLevelsArraySorted = userLevelsArray.sort((a, b) => a == b ? b.xp - a.xp : b.level - a.level)

    const userLevelDataIndex = userLevelsArraySorted.findIndex((user) => user.id == userId)

...
__export(constants_exports, {
  ...
  isDevEnv: () => isDevEnv,
  keyPermissionsArray: () => keyPermissionsArray
});
var isDevEnv = process.env.NODE_ENV == "development";
var keyPermissionsArray = [
  "Administrator",
  "ManageGuild",
  "ManageRoles",
  "ManageChannels",
...
async function alertDevs(data) {
  if (typeof process.env.WEBHOOK_ALERT != "string")
  ...
  const webhookAlert = await webhookClient.send({
    embeds: [
      new Discord.EmbedBuilder().setTitle(emoji + data.title).setDescription(data.description ?? null).setColor(data.type == "message" ? enums_exports.PhaseColour.Primary : data.type == "warning" ? enums_exports.PhaseColour.Warning : enums_exports.PhaseColour.Failure).setTimestamp().setFooter({ text: constants_exports.isDevEnv ? "Phase [Alpha]" : "Phase [Production]" })
    ]
  });

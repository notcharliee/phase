 ...
 * @param error The error string (common errors found in {@link Utils.PhaseError}).
 */
export function clientError <deffered = false> (
  ...
  const throwError = (error: any) => Utils.alertDevs({
...
export function clientSlashCommand ({ data, permissions, execute }: {
  data: Utils.SlashCommandData<true>,
  permissions?: Utils.SlashCommandPermissions,
  execute: (
...
}): Utils.EventFile<'interactionCreate'> {

 ...
 *  this will be removed in a future update VERY SOON.
 */
...
export default async (client: Discord.Client<true>) => {
  ...
  async function sendlog (guildId: string, embed: Discord.EmbedBuilder) {
    ...
    const guildSchema = await Schemas.GuildSchema.findOne({ id: guildId })
    ...
    const channel = client.channels.cache.get(guildSchema.modules.AuditLogs.channels.server)!

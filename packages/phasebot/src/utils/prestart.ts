...
export const getPrestart = async () => {
  const prestartURL = pathToFileURL(resolve(process.cwd(), "build/prestart.js"))

  if (!existsSync(prestartURL)) return () => undefined

  const prestartFunction = await (await import(prestartURL.toString())).default as unknown
